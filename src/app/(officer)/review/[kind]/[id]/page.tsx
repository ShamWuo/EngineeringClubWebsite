    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      <div>
        ...
        <div className="flex items-center gap-2 mb-1">
          <Badge variant="outline" className="text-3xs uppercase font-bold tracking-wider bg-zinc-900 border-zinc-700 text-zinc-300">
            {kind} Request Review
          </Badge>
        ...
        <p className="text-xs text-zinc-400 mt-0.5">
          Submitted by <strong className="font-semibold text-zinc-200">{requester?.full_name || requester?.email}</strong> ({requester?.email}) on {new Date(requestData.created_at).toLocaleString()}
        </p>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="border-zinc-800 bg-zinc-950">
            <CardHeader className="pb-3 border-b border-zinc-850">
              <CardTitle className="text-base font-bold text-white">Proposal Details & Rationale</CardTitle>
            </CardHeader>
            <CardContent className="pt-4 space-y-4 text-sm leading-relaxed text-zinc-200">
              <div className="bg-zinc-900/80 p-4 rounded-xl border border-zinc-800 whitespace-pre-wrap text-xs text-zinc-300">
              ...
              {kind === 'general' && (
                <div className="grid grid-cols-2 gap-3 text-xs pt-2 border-t border-zinc-800">
                  <div>
                    ...
                    <strong className="font-semibold text-zinc-200 capitalize">{requestData.category}</strong>
                  </div>
                  <div>
                    <span className="text-zinc-500">Urgency: </span>
                    <strong className="font-semibold text-red-400 capitalize">{requestData.urgency}</strong>
                  </div>
                </div>
              )}
              ...
              {kind === 'team' && comp && (
                <div className="text-xs space-y-1">
                  ...
                  <strong className="font-semibold text-white">{comp.name}</strong>
                </div>
              )}
                ...
                <div className="space-y-2 pt-2 border-t border-zinc-800">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {proposedMembers.map((m) => (
                      <span key={m.id} className="text-xs bg-zinc-900 border border-zinc-800 text-zinc-300 px-2.5 py-1 rounded-md">
                        {m.full_name || m.email}
                      </span>
                    ))}
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3 text-xs pt-2 border-t border-zinc-800">
                  {requestData.organizer && (
                    <div>
                      ...
                      <strong className="font-semibold text-white">{requestData.organizer}</strong>
                    </div>
                  )}
                  {requestData.url && (
                    <div>
                      <a href={requestData.url} target="_blank" rel="noopener noreferrer" className="text-red-400 hover:underline flex items-center gap-1">
                  ...
                  {requestData.estimated_cost_cents !== undefined && (
                    <div>
                      ...
                      <strong className="font-semibold text-red-400">${(requestData.estimated_cost_cents / 100).toFixed(2)}</strong>
                    </div>
                  )}
                  {requestData.estimated_team_size && (
                    <div>
                      <span className="text-zinc-500">Est. Team Size: </span>
                      <strong className="font-semibold text-white">{requestData.estimated_team_size} members</strong>
                    </div>
                  )}
                </div>
              )}
              ...
              {kind === 'workshop' && (
                <div className="grid grid-cols-2 gap-3 text-xs pt-2 border-t border-zinc-800">
                  <div>
                    ...
                    <strong className="font-semibold text-white">{requestData.offering_to_teach ? 'Yes (Requester volunteering to teach)' : 'No (Requesting expert / peer)'}</strong>
                  </div>
                  {requestData.preferred_timeframe && (
                    <div>
                      <span className="text-zinc-500">Preferred Timeframe: </span>
                      <strong className="font-semibold text-white">{requestData.preferred_timeframe}</strong>
                    </div>
                  )}
                </div>
              )}
                ...
                <div className="space-y-3 pt-2 border-t border-zinc-800">
                  ...
                  <div className="overflow-x-auto rounded-lg border border-zinc-800">
                    <table className="w-full text-left text-xs">
                      <thead className="bg-zinc-900 text-2xs uppercase text-zinc-400 font-semibold border-b border-zinc-800">
                        <tr>
                      ...
                      <tbody className="divide-y divide-zinc-850">
                        {lineItems.map((item) => (
                          <tr key={item.id}>
                            <td className="py-2 px-3 font-medium text-zinc-200">{item.description}</td>
                            <td className="py-2 px-3 text-zinc-400">{item.vendor || 'N/A'}</td>
                            <td className="py-2 px-3 text-right">${(item.unit_cost_cents / 100).toFixed(2)}</td>
                            <td className="py-2 px-3 text-center">{item.quantity}</td>
                            <td className="py-2 px-3 text-right font-semibold text-red-400">${((item.unit_cost_cents * item.quantity) / 100).toFixed(2)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                  ...
                  {attachments.length > 0 && (
                    <div className="pt-2 text-xs">
                      ...
                      {attachments.map((a) => (
                        <span key={a.id} className="inline-flex items-center gap-1 ml-2 text-red-400 font-medium">
