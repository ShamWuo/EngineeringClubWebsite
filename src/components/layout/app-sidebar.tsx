...
export function AppSidebar({
  ...
  return (
    <aside className="w-64 shrink-0 hidden md:flex flex-col border-r border-zinc-850 bg-black min-h-[calc(100vh-4rem)] p-4 justify-between">
      <div className="space-y-6">
        {/* Member Navigation */}
        <div>
          <div className="text-2xs font-bold text-zinc-500 uppercase tracking-widest px-3 mb-2 font-mono">
            Club Spaces
          </div>
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    isActive
                      ? 'bg-red-950/70 text-red-400 border border-red-800/80 shadow-md shadow-red-950/40'
        ...
        {isOfficer && (
          <div>
            <div className="text-2xs font-bold text-zinc-500 uppercase tracking-widest px-3 mb-2 flex items-center justify-between font-mono">
              <span>Officer Tools</span>
              <span className="text-3xs bg-red-950 text-red-400 border border-red-800 px-1.5 py-0.2 rounded font-bold">
                OFFICER
              </span>
            </div>
            <nav className="space-y-1">
                ...
                return (
                  ...
                  >
                    ...
                    {item.badge !== undefined && (
                      <span className="flex h-4 px-1.5 items-center justify-center rounded-full bg-red-600 text-white font-bold text-3xs shadow-sm">
                        {item.badge}
                      </span>
                    )}
                  </Link>
      ...
      {primaryLinks.length > 0 && (
        <div className="pt-4 mt-6 border-t border-zinc-850">
          <div className="text-2xs font-bold text-zinc-500 uppercase tracking-widest px-2 mb-2 font-mono">
            Pinned Hubs (Tier 1)
          </div>
          <div className="space-y-1">
            {primaryLinks.map((link) => (
              <a
                ...
                className="flex items-center justify-between px-2.5 py-1.5 rounded-md text-xs font-medium text-zinc-300 hover:bg-zinc-900 hover:text-red-400 transition-all group"
              >
                ...
                <ExternalLink className="h-3 w-3 text-zinc-600 group-hover:text-red-400 opacity-0 group-hover:opacity-100 shrink-0" />
              </a>
            ))}
          </div>
        </div>
